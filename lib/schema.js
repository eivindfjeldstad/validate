...
class Schema {
  ...
  typecast(obj, props = this.generateProps(obj)) {
    ...
    Object.keys(props).forEach(key => {
      ...
      const casted = prop.typecast(value);

      if (casted === value) return;

      dot.set(obj, key, casted);
      counter++;
    });
  ...
  strip(obj, props = this.generateProps(obj), prefix) {
    Object.keys(obj).forEach(key => {
      ...
      if (!props[path]) {
        // Delete is safe to use because all array indices should be
        // defined as paths in the props object
        delete obj[key];
        return;
      }
   ...
   *
   * @param {Object} [obj] - an object containing new messages
   * @return {Schema|Object}
   */

  messages(obj) {
    if (!obj) return this._messages;
    Object.keys(obj).forEach(k => this._messages[k] = obj[k]);
    return this;
  }
   ...
   *
   * @param {Object} [obj] - an object containing new validators
   * @return {Schema|Object}
   */

  validators(obj) {
    if (!obj) return this._validators;
    Object.keys(obj).forEach(k => this._validators[k] = obj[k]);
    return this;
  }
...
function generatePaths (path, obj, orig='', prefix = '', map={}) {
  // Return early if there are no arrays
  if (!path.includes('.$')) {
    map[prefix + path] = orig + path;
    return map;
  }
  ...
  arr.forEach((val, i) => {
    const newPath = `${prefix + first}.${i}`;
    map[newPath] = `${orig + first}.$`;
    generatePaths(parts.join('.$'), obj, map[newPath], newPath, map);
  });

...
class Property {
  ...
  async validate(value, ctx, path = this.name) {
    const types = this.registry.keys();
    const done = {};
    let error;

    // Required first
    if (error = await this._run('required', value, ctx, path)) {
      return error;
    }
    ...
    // Run type second
    if (error = await this._run('type', value, ctx, path)) {
      return error;
    }
  ...
  async _run(type, value, ctx, path) {
    if (!this.registry.has(type)) return;

    const {args, fn} = this.registry.get(type);
  ...
  _register(type, args, fn) {
    this.registry.set(type, { args, fn });
    return this;
  }

...
class Property {
   ...
   * prop.nonempty()
   *
   * @param {Boolean} [bool] - `true` if nonempty, `false` otherwise
   * @return {Property}
   */

  nonempty(bool = true) {
    return this._register('nonempty', [bool]);
  }
  ...
  validate(value, ctx, path = this.name) {
    ...
    // Nonempty
    if (error = this._run('nonempty', value, ctx, path)) {
      return error;
    }
    ...
    // Make sure required, nonempty and type are not executed again
    done.required = true;
    done.nonempty = true;
    done.type = true;
